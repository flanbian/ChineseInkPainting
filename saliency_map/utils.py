...
class OpencvIo:
    ...
    def imwrite_array(self, images):
        for x in images:
            cv.imwrite("saliencyMap.png", np.uint8(self.__util.normalize_range(x)))
            
    def imget_array(self, images):
        for x in images:
    	  	  return np.uint8(self.__util.normalize_range(x))
